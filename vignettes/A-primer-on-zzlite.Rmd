...
The following section serves as a simple introduction to the `zzlite` package. Since Zamzar requires authentication, it is assumed that you have a valid (and nonexhausted) key at your disposal. If not, go signup via [Zamzar](https://developers.zamzar.com/)s website to get one. Otherwise, keep on reading.  
...
2. By leveraging an `.Renviron` file. Simply create a variable called `ZAMZAR_USR` in your `.Renviron` and pass it a Zamzar key. If done succesfully, you can skip passing your key to the `usr` param whenever you're invoking `zzlite` functions.  
...
1. If you're curious about the formats the Zamzar API can convert for you, do:
...
2. If you're curious about what a given format can be converted to, as well as the cost (in Zamzar credit), pass the origins extension, e.g.:  
...
The prod bool decides whether or not we should use a development endpoint or a production endpoint.  
...
So how do we obtain a file after we've posted it to the Zamzar API and requested conversion? `zz_get_info()` to the rescue!  

Assuming that we've just posted the `avatar.emf` to Zamzar and have received a `201` response from the API, a subsequent invocation of `zz_get_info()` will return a table like this:
...
And as expected, Zamzar is done with the conversion, and a file with id `61920045` and an extension of `png` is now in the top of our stack of files associated with our Zamzar account.  

Sometimes it is nice to inspect the entire stack of files for your Zamzar account. This is also possible, just switch the parameter `latest` from `TRUE` to `FALSE` (the parameter defaults to `TRUE`):
...
Now, how do we _get_ a file from the stack?

### `zz_get()`

We can get back a file by invoking `zz_get()` with appropriate arguments.  
...
Lets extend a bit on the two `zz_get*` functions.  



_This is where we're going to elaborate on the zz_get* functionality_

---

> *Please note that the package has a strict focus on rather simple conversions via Zamzar (implied by the _lite_ suffix in the name). It is intentional that the package doesn't focuse on deletion of files, cloud integration etc.  