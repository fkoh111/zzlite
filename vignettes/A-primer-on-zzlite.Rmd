...
1. Either by passing a hardcoded Zamzar key as an argument to the `usr` parameter.  

2. Or by leveraging an `.Renviron` file. Simply create a variable called `ZAMZAR_USR` in your `.Renviron` and pass it a Zamzar key. If done succesfully, you can skip passing a key to the `usr` param whenever you're invoking `zzlite` functions.  
